...
class Deals extends React.Component {
  ...
  fetchData = () => {
    axios.get('http://localhost:4001/api/destinations')
      .then(response => {
                      ...
                      <div className="card-wrapper container-sm d-flex justify-content-around">
                        {group.map(item => (
                          <div key={item.id} className="card discounts-carousel-card card-exclusive m-3">
                            <img src={`../images/cities/${item.images}`} className="card-img-top img-deals rounded carousel-img" alt={item.city} />
                            <div className="card-deals-body mb-1">
                              <div className="row my-2 pe-1">
                                <div className="col-md-6">
                                  <h5 className="card-title card-deals-title">{item.city}</h5>
                                </div>
                                <div className="col-md-6 d-flex justify-content-end">
                                    ...
                                    <span className="text-muted">{item.qualification}</span>
                                </div>
                              </div>
                              <div className="row my-2 pe-1">
                                <div className="col-md-6">
                                ...
                                <div className="col-md-6 d-flex justify-content-end">
                                  <span className="card-text oldprice text-muted">${item.rates}</span>
                                  <span className="card-text price ms-2">${item.discounts}</span>
                                </div>
                              </div>
                            </div>
                          </div>
